...
function toHex(red: number, green: number, blue: number) {
    return [
        ...
        green,
        blue
    ].map(el => el.toString(16).toUpperCase().padStart(2, "0"));
}
    ...
    return (
        <ContentSlide Title="Hex Colors" context={ context } Content={
            <div className={ classes.colorContainer}>
                <div style={{ backgroundColor: `#${hexRed}${hexGreen}${hexBlue}`, 
                              color: inverseColor,
                    ...
                    <code>
                        ...
                        { [hexRed, hexGreen, hexBlue].map(colorComp => (
                            <span className={ classes.colorComponent }
                                  style={ { borderColor: inverseColor } }>
                                { colorComp }
                            </span>
                        )) }
                    </code>
                ...
                <div className={ classes.colorAdditionRow }>
                    <div className={ classes.colorAdditionBlock } 
                         style={{ backgroundColor: `#${hexRed}0000` }}></div>
                    <code>+</code>
                    <div className={ classes.colorAdditionBlock } 
                         style={{ backgroundColor: `#00${hexGreen}00`}}></div>
                    <code>+</code>
                    <div className={ classes.colorAdditionBlock } 
                         style={{ backgroundColor: `#0000${hexBlue}`}}></div>
                </div>
            </div>
        }/>
    )
